...
import { createAcademyWorkflow } from "../../../workflows/academy/create-academy";
import { z } from "zod";
...
export const POST = async (
  ...
  const { result } = await createAcademyWorkflow(req.scope).run({
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const { data: academies, metadata: { count, take, skip } = {} } =
    await query.graph({
      entity: "academy",
      ...req.queryConfig,
    });
